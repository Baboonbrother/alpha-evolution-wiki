...
這一頁承載 owner **第三輪批評**（2026-07-22）的核心修正。前兩輪把「演化什麼」（世界模型不是策略）與「怎麼分開裁決」（三迴圈）修對了，但整份 wiki 的主線仍然**斷成兩段**：一段是 owner 手上真的在賺錢的最強策略 king2（活在王牌真錢線，wiki 幾乎不談它）；另一段是信念契約支線（[實驗 004](exp-004-belief-contract.md) 拿 MIEE 事件假說跑通了到期對帳——但那兩條信念跟任何真實決策都沒有關係）。兩段各自成立、互不相接。**修法＝建立「現任冠軍」制度：把 king2 凍結成研究帳上的冠軍基準，讓每一個研究問題從冠軍的決策殘差長出來、每一個挑戰者以冠軍為分母對決、晉升只以追加新列發生——冠軍永不覆寫。**

> **認知答案**：一個活的研究系統需要一個「現任冠軍」——當前已知最好、被凍結、不可被悄悄改動的決策政策——因為只有對照冠軍，「更好」才有明確定義（Δ＝挑戰者 − 冠軍），研究問題才有天然的決策相關性（冠軍上次真實決策錯在哪，就是最值得研究的未知）。
>
> **行動答案**：主線＝**凍結冠軍 → 決策殘差 → 世界假說 → 預註冊 → 挑戰者 → 樣本外對決 → 晉升**。目前真的完成的是前兩步加預註冊（凍結列 `champion_registry` id=2、殘差資料集 5,409 列、[實驗 005](exp-005-king2-prereg.md) 五臂預註冊）；假說尚未從殘差長出、零個挑戰臂跑過、零次對決、零次晉升。**不要把本頁讀成「世界信念已經改善了 king2」——那一步一次都還沒發生。**
...
- **研究問題有了天然的決策相關性。** [假說引擎](hypothesis-engine.md) 第二輪用 ResearchValue 的 DecisionRelevance 項防「研究不改變任何決策的問題」——但那一項是引擎自估的，可以吹。冠軍殘差不用估：**每一列殘差都是一筆真的虧掉或漏掉的錢**。「2609 陽明在候選池排 43 落選、隨後三週漲 133%」這個問題的決策相關性不需要任何人打分。
...
| 角色 | 定義 | 存在的理由 | 對應 [實驗 005：king2 冠軍—挑戰者五臂預註冊（REGISTERED，零臂已跑）](exp-005-king2-prereg.md) 的臂 |
|---|---|---|---|
...
四條分支的優先序不是拍腦袋：**改善與失效兩支直接踩在殘差資料上**（每列都是真錢的錯），先走；解釋是它們的地基（不懂為什麼賺，就不知道哪些錯是本質的）；新策略最後（它連分母都還要借冠軍的）。這就是 [假說引擎](hypothesis-engine.md) 第三版「殘差優先、ResearchValue 退為補充」的制度來源。
...
對應 [實驗 005](exp-005-king2-prereg.md) 預註冊的「殘差四格（選中×漲跌）」：長假說的來源限定在**做錯決策的兩格**——假陽性（選中且跌＝①）與漏網（沒選且漲＝②）；③屬持有層（接 [持有期生命週期](fw-holding-lifecycle.md) 的地盤）、④屬 regime 層。
...
延伸：五臂設計、晉升五道門與 blocked 誠實原因見 [實驗 005](exp-005-king2-prereg.md)；殘差怎麼變成研究問題見 [假說引擎](hypothesis-engine.md)；冠軍挑戰環怎麼疊在 W/O/B/P 主軸上見 [研究迴圈](research-loop.md)；信念的預註冊與到期對帳機件（本制度的④⑥兩步所依賴）見 [信念契約](world-belief-contract.md) 與 [實驗 004](exp-004-belief-contract.md)；三迴圈各自的裁判見 [三個迴圈](three-loops.md)。
...
冠軍制度回答「要不要看多這家公司」；選對之後「用什麼報酬形狀表達」是另一層決策，見 [載具路由器](instrument-router.md)（股票／權證／CB／CBAS 的路由原則）與 [實驗 006](exp-006-cb-router-prereg.md)（CB 載具路由第一實驗，構想級）。
