...
**研究雙語（Research Bilingual / Research Ontology + Cognitive Compiler）**是[五層量化語言](lang-quant.md)的第四層，也是所有實驗的**出口**。它的名字裡的「雙語」很實在——系統其實同時需要兩種語言，中間靠一台確定性編譯器把兩邊接起來：
...
第一套語言防止**研究混亂**（LLM 亂造假說、亂比較），第二套防止**報告混亂**（把推論寫成事實、語氣超過證據）。前三層（[特徵代數](fw-feature-algebra.md)／[世界訊號](fw-world-signal.md)／[持有期](fw-holding-lifecycle.md)）都在講研究語言這一側；這一層額外補上認知語言，並用編譯器保證「同一份實驗自動編譯成固定認知順序的人類報告」。
...
- **證據五級 E0–E4**：每個結論必標「有效到哪一級」。E0＝想法（沒回測）、E1＝樣本內、E2＝重複支持但無樣本外、E3＝樣本外確認、E4＝前瞻/實戰確認。這對映 [策略本體論](overview.md)的「回測是在提高期望的證據等級」。
...
這四台已真跑的引擎報告——[實驗 000](exp-000-engine-first-run.md)、[實驗 001](exp-001-candidate-c.md)、[實驗 002](exp-002-ablation.md)、[實驗 003](exp-003-graph-evolution.md)——全部按 fixed-order v2 骨架寫成，每一份都有「0 決策首頁／決策欄位表／一眼看懂／分命題信心／由門推出的裁決／升降級表」。它們是這一層要保證「所有實驗都長這樣」的活範例。特別值得看的是它們如何實踐「語氣不超過證據」：[實驗 001](exp-001-candidate-c.md)生出一個 33% 年化的漂亮結果，報告卻替它掛三張警告標籤、裁決 provisional——這就是編譯器該做的事（見 [誠實紀律](discipline.md)）。

## 這一層負責的兩本字典，值得單獨記住

- **評估四層（第 7 本字典）**：訊號證據／組合證據／穩健證據／認知證據——這是「一個 Alpha 的證據要分幾種來看」的封閉分類，[持有期](fw-holding-lifecycle.md)的剩餘 Alpha 就是要餵進這四層量測的對象。
- **失效字典（第 8 本，8 類）**：專門用來「攻擊自己的研究」——列出一個結論可能怎麼假。這對映進化迴圈的[攻擊閘](method-gates.md)與失敗歸因。
...
延伸閱讀：這一層在整個引擎裡的位置見 [StrategySpec](method-strategy-spec.md)（假說的 features 欄存 [特徵代數](fw-feature-algebra.md) DSL、行動決策表吃 [世界訊號](fw-world-signal.md)九態）與 [十道證據閘](method-gates.md)。所有五層共用的誠實紀律見 [方法論：誠實紀律](discipline.md)。

---

**被連結自（反向連結）：** [方法論：誠實紀律（拒絕相信自己）](discipline.md) · [方法：策略基因（StrategySpec 九部件）](method-strategy-spec.md) · [方法：部件從哪取用、怎麼啟用](method-components.md) · [框架：持有期生命週期](fw-holding-lifecycle.md) · [框架：時間層（時態邏輯節點）](fw-temporal.md) · [框架：特徵代數](fw-feature-algebra.md) · [總覽：從一個念頭到一台會拒絕相信自己的引擎](overview.md) · [詞彙表](glossary.md) · [量化結構組成語言（總覽）](lang-quant.md) · [首頁：Alpha 進化迴圈研究 Wiki](index.md)
