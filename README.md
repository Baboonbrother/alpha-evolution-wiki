...
> 以世界模型為主軸的 Research OS：現任冠軍（凍結 king2）→ 決策殘差 → 世界假說 → 信念契約 → 樣本外對決 → 晉升；載具路由器（股票/CB/CBAS＝報酬形狀）。每次實驗（000–009、F01、F02）逐環節透明。exp-007＝第一條從冠軍殘差長出的世界假說 B-RES-001，裁決 NARROW_SCOPE（誠實：未確認也未否證，C 臂維持 blocked）。
...
owner 裁決：王牌不只是冠軍基準，**是整個探索空間的母體**。暫停 Polymarket／本地模型 operator／通用世界事件擴張，3-5 輪集中「FinLab × king2 殘差」。已完成：[EXP-F01/F01.1](exp-f01-king2-overlap.md)——完整 king2 重建為可計算節點（對凍結 parquet 逐格對帳）＋法人家族 11 個 FeatureSpec 化變化型特徵＋重疊圖（**10 LOW_OBSERVED_OVERLAP＋1 PARTIAL_OVERLAP＋0 換皮**，含符號一致率與非線性 OOF 檢定）＋定義圖/證據圖分表＋[可驗證證物包](evidence/F01/run_manifest.json)。**尚無任何新 Alpha 宣稱**。**F02 已跑完（F02.1 降級口徑）**：[EXP-F02](exp-f02-residual-discrimination.md)——把 11 個法人特徵加進 king2 鏡像 base model，6 折 walk-forward（訓練窗門檻／embargo／grouped split，預註冊凍結）**雙任務 NOT_DETECTED_UNDER_F02_SPEC**：base 本身有辨識力（PR-AUC ≈1.5–2× 盛行率），在「月頻錨／all-11 整包／線性邏輯迴歸／固定 λ／絕對尾端二元標籤」spec 下未偵測到法人增量；合成雙向測試證明機器有檢定力。**證明的是「這個鏟法沒挖到礦」，不是「法人這座山沒有礦」**（無 CI／等效區間；稀釋、λ 失配、共線等成因分不開）——spec 綁定入圖邊，統計補強收進矩陣引擎。[證物包](evidence/F02/run_manifest.json)。**同日冠軍身分對帳發現 base＝AARO 鏡像而非 live 王牌 STR_king1**——見最新一節。
...
> - 編號實驗 **12 個**（000–009＋F01/F01.1/F02）；信念契約 **6 條**＝策展 3＋自主示範 3；從冠軍殘差長出的世界假說 **1 條**（B-RES-001，NARROW_SCOPE）
