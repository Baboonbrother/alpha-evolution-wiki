...
但**世界鏈到「產業營收世界狀態」為止就斷了**——運價（BDI/SCFI）、新聞事件節點、多跳供應鏈（船舶供給／港口壅塞）仍**未接入**（是資料取得專案、非資料不存在，見 [[blockers|難點]] B1），模組把這些明白列在 `world_chain.unfilled` 裡，不假裝有。所以 v2 把 owner 要的鏈補到了「產業營收」這一跳，前面「運價 → 產業營收」和旁邊「新聞 → 世界狀態」那幾跳還是空的。

## B0 更正：這一頁曾寫「交互全 independent」——那是語義錯誤，已修

owner 揪出本頁（與整條線）最嚴重的問題：上面那些交互檢定 n=8、17、25、t 全不顯著，卻被寫成 `independent` 並封閉了圖缺口——**「未拒絕交互」不等於「證明獨立」**，同一台機器的 `min_n=20` 在信念結算會判 `HOLD_PRIOR`，這裡卻拿更少的樣本宣稱了更強的結論。圖裡因此有 6 條假獨立邊，系統誤以為那些格子已解決。

修正（`wm/evidence.py`＝交互裁決唯一入口，考卷 15/15）：

- **`independent` 要等效性檢定才配**：效應 95% CI **整段**落在事前指定的無經濟意義區間（報酬單位 ±0.002 ≈ 半個來回成本；無因次 ±0.01）。不顯著且 CI 超出 band → `inconclusive(UNDERPOWERED)`；n<20 → `inconclusive(LOW_N)`，樣本不足什麼都不能宣稱。
- **inconclusive 不封閉缺口**：格子三態 `OPEN / AWAITING_MORE_DATA / CLOSED_CONCLUSIVE`——inconclusive 進 AWAITING（不封閉、也不同資料重測，重新可測靠 LIVE_FORWARD 新資料），只有結論性裁決封閉。
- **6 條假獨立邊以 `edge_supersession` 更正**：舊列不刪（append-only）、新列 `-c1` 接手、supersession 關係入帳。重算：4 條 `LOW_N`、2 條 `UNDERPOWERED`（如 n=25 那條的 CI [−0.034, +0.038] 遠超 ±0.002）——**0 條真獨立**。schema 遷移把 `inconclusive` 加進 relation CHECK（逐列 sha 比對、append-only 觸發器重掛）。
- **spec_hash／run_hash 拆開**：spec_hash＝假說身分、run_hash＝資料＋程式＋本次執行指紋，都入帳。
- **統計設計回應（B4 第一步）**：新增第三種可編譯 form `continuous_interaction`（事件內連續交互回歸，不做中位切）。

修正後真跑三輪：新裁決全部 `inconclusive(LOW_N)`——**誠實說「現有資料測不出來」，而不是宣稱「已知獨立」**；格進 AWAITING、OPEN 遞減不迴圈。所以上表的「→ independent」全部應讀作「→ inconclusive（證據不足）」；本頁保留原表作歷史記錄，帳上以更正列為準。

### B0 修正也對抗查證過：裁決 FIXED（三鏡頭全 SOLID）

三個獨立稽核鏡頭實跑攻擊 B0 修正，綜合裁決＝**FIXED**：

- **獵假獨立（最關鍵）**：窮舉邊界攻擊（n=19/20、CI 端點恰等 band、std=0/負/NaN/inf、mean=NaN、單位錯配、form 錯標借寬 band）——**無法經活路徑構造出任何「效應可能有經濟意義卻判 independent」的輸入**。CI 數學逐位驗算吻合；band 的單位語義驗證過（excess 是小數報酬、0.002=0.2%≈半個來回成本）。兩個部分得手的攻擊都有量化上界且蓋不過來回成本：legacy 反推的捨入窗（|t|∈[0.005,0.01) 可縮 CI 至 2x）與 z=1.96 對 t 分布的近似（n=20 時窄 6.8%，仍比業界 TOST 的 1.729 嚴格）——**兩者均已防禦性封死**（|t|<0.02 拒判＋docstring 精度聲明）。
- **封閉語義**：inconclusive 只產 AWAITING、真跑 OPEN 嚴格遞減 [5→0] 後誠實收斂（附「awaiting≠closed」註記）、收斂後再呼叫不迴圈。**反事實驗證**：在拋棄式副本刪光 supersession 記錄後，4 格立刻翻成 CLOSED——證明防誤封真的來自 supersession 機制，不是資料巧合。
- **遷移完整性**：備份 7 列與現 DB 逐欄比對零遺失零竄改；6 條舊 independent 列原樣保留、各配 `-c1` 更正列、supersession 6 筆一一對應；兩表 append-only 實測擋 UPDATE/DELETE。備份已長期化至 `data/backups/`（原在 /tmp 重開機即失，稽核建議已修）。

稽核同時誠實列出**已聲明的未完成**：AWAITING 格目前永久等待——「LIVE_FORWARD 新資料到位時自動重測」的掛鉤還沒建，這是 AWAITING 語義要完整兌現的下一塊。
