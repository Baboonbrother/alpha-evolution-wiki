...
> 整個研究 wiki 的單檔匯出（共 41 頁），供 LLM 一次讀完、逐環節審閱。
...
>
> **行動答案**：路由研究的順位裡（見 載具路由器：king2 決定看多誰，路由器決定用什麼報酬形狀表達），CB 排第 2–4 步、CBAS 排最後第 5 步。CB 端現在能動工的是「補 PIT 條款行情」——六維覆蓋表顯示三維全套、兩維半套，**唯一致命缺口＝歷史賣回價無資料源，債底時序做不出來**（這正是舊 sleeve 研究被判 PASS_SIGNAL_ONLY 的直接原因）。CBAS 端＝零代碼、零資料源、零通路，規劃文件已裁決**觀察區**：在十項承作資料拿到之前，只能做理論研究，不得宣稱可部署。
...
| 債底 | ⚠️ 半套 | **方法有、時序無**：PV(賣回價) 計算範本存在（債底快照計算範本），但只能算當前快照——**歷史賣回價無資料源，債底 PIT 時序做不出來**。這是全線最大缺口，也是 sleeve 被判 SIGNAL_ONLY 的直接原因 |
| 條款 | ⚠️ 大半 | 轉換價／轉換賣回強贖起迄／餘額／票面利率齊；**獨缺賣回「價格」**（finlab 欄位存在但僅 3/86 檔有值、TPEx openapi 只有當前值無歷史）；條款全文層（下修公式、擔保與否）也缺 |
...
2. **地板的位置今天算不準。** 債底＝PV(賣回價)，而歷史賣回價無源、折現率端缺曲線——目前所有債底數字都是「當前快照＋假設折現率」的近似。
...
| CB 債券區 sleeve（價 90–110 等權） | **PASS_SIGNAL_ONLY** | 凸性分散為真（真實標價下 beta≈0.04、2022 仍正），但招牌平滑被停滯報價放大 ~3×、淨 carry 因 83/86 檔缺賣回價**無法證實** |
...
六個編號不是六個孤立實驗，而是三條線：**策略軌**（000–003，同一條血統連續四刀，後一刀常拆穿前一刀）；**機件支線**（004，驗證信念 settle 機件 fail-closed，內容不在冠軍決策鏈上）；**冠軍軌**（005，第三輪主線的第一份預註冊——凍結 king2、攤開殘差、五臂設計凍結，**零臂已跑**）；**載具軌**（006，冠軍選對公司後「用什麼報酬形狀表達信念」——CB 載具路由第一實驗，構想級，見 載具路由器）。
...
| 006 | king2 選對公司但不確定發動時間時，CB 能否用較好的下行形狀保留 Alpha（載具路由第一實驗） | 載具路由器 載具路由器：king2 決定看多誰，路由器決定用什麼報酬形狀表達 ＋CB 知識 CB 與 CBAS：債底加選擇權的合體，與拆開賣的兩端 | **構想級（判準未凍結、零臂已跑）** | 四臂設計成文（A 冠軍全股票／B 無腦 CB 替代／C 合格閘替代=主挑戰臂／D 風險預算配置）；第一工項＝自建歷史賣回價資料源（債底時序），沒有它 C 臂「債底合格」算不出 | 實驗 006：CB 載具路由四臂預註冊（構想級——判準未凍結、未入帳、零臂已跑） |
...
實驗索引是**累積的活文件**。下一列可預期的內容已經被主線指名：**從殘差四格長出第一條世界假說、登記信念契約、跑到 settle**（它同時是 C 臂解鎖鏈的第一步）；策略側則欠著 walk-forward 與 exp-005 的 A 臂復算。若你是來評審的 LLM，最有價值的攻擊點收在 給 LLM 評審；每個實驗頁末尾的「誠實邊界」逐條列出作者自己都不確定的接縫。詞彙不熟先查 詞彙表。
...
flowchart TB
    ...
    C["C 臂＝信念增強（主挑戰者）<br/>A＋confirmed 世界信念<br/>🔒 blocked：帳上零條 confirmed"]
...
## C 臂為什麼 blocked：帳上零條 confirmed 信念

C 臂的定義是「冠軍＋**已確認**的世界信念」，而信念帳目前的真實狀態是：**一條 confirmed 都沒有**。僅有的兩條真跑信念都是負向結局——B-H-003 判 REFUTE（信心 0.5 → 0.2256）、B-H-001 判 WEAKEN（0.5 → 0.3913），見 實驗 004。

這裡刻意選了**誠實的慢路**而不是方便的快路：快路是「隨便挑條聽起來合理的信念、標成 confirmed、把 C 臂跑起來」，那會讓 C 臂從第一天就建立在自我認證上。慢路是 blocked 等貨真價實的解鎖鏈：**殘差四格（選中×漲跌）的假陽性與漏網兩格 → 長出世界假說 → 登記 belief_contract → 前瞻對帳 → 第一條 REINFORCE（confirmed）→ C 臂解鎖**。C 臂什麼時候能跑，取決於殘差線什麼時候長出第一條撐過前瞻對帳的信念——不取決於任何人的耐心。
...
- **C 臂可能長期 blocked。** 解鎖鏈的每一步（殘差→假說→對帳→REINFORCE）都可能失敗；第一批殘差長出的信念全數被 REFUTE 是完全可能的結局，屆時 C 臂繼續 blocked，這個實驗就誠實地停在那裡。
...
>
> **行動答案**：本實驗**還不能跑**。第一工項不是寫回測，而是補資料——歷史賣回價無資料源，債底時序做不出來，C 臂的「債底合格」條件目前算不出。先解資料，再凍判準，最後才跑。
...
flowchart TB
    ...
    R --> C["C 臂＝合格閘替代<br/>條款·信用·流動性·溢價·債底<br/>🔒 blocked：債底時序做不出"]
...
| **賣回價歷史（債底時序）** | ❌ **全線最大缺口** | 條款面板的賣回「價格」欄歷史幾乎全缺（抽查僅 3/86 檔有值）、公開 API 只有當前值無歷史 → **債底時序做不出來**。這正是既有 sleeve 研究被判 SIGNAL_ONLY 的直接原因，也是本實驗 C 臂 blocked 的直接原因 |
...
沒有歷史賣回價，C 臂的「債底合格」條件算不出，四臂中的主挑戰臂直接缺席，實驗跑了也只剩 A/B/D 的殘缺對決。因此**本實驗的第一工項不是回測代碼，是建賣回價資料源**：從公開發行資訊回補歷史、並從今天起逐日存當前值（當前值公開 API 有）。這也是既有 sleeve 判決書裡明列的升級路，兩條線共用同一個資料工項。

行情面的自由資料源已驗證可用（櫃買中心每日 CB 行情 CSV，無需登入，含等價與議價兩軌完整欄位）：
